                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-start gap-2 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-gray-900 truncate text-xs" title={t.name}>
                        {t.name}
                      </h3>
                      <div className="text-[10px] text-gray-500">{renderStars(t.rating)}</div>
                    </div>
                  </div>
        ...
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-2 text-left text-[10px] font-medium text-gray-500 uppercase tracking-wider">Rating</th>
            ...
            {currentTestimonials.map((t, index) => (
              t && t.content && t.name && t.rating !== undefined && t.userId ? (
                <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-1.5 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="max-w-40">
                        <div className="font-medium text-gray-900 truncate text-xs" title={t.name}>
                          {t.name}
                        </div>
                      </div>
                    </div>
                  ...
                  <td className="px-4 py-1.5 whitespace-nowrap">
                    <div className="text-xs text-gray-600">{renderStars(t.rating)}</div>
                  </td>
                  <td className="px-4 py-1.5">
                    <div className="max-w-xs">
                      <p className="text-xs text-gray-600 truncate" title={t.content}>
                        {truncateText(t.content, 50)}
                      </p>
                    </div>
                  </td>
                  <td className="px-4 py-1.5 whitespace-nowrap">
      ...
      <div className="h-full overflow-y-auto mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-medium text-gray-500">Total Testimonials</p>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-medium text-gray-500">Filtered Testimonials</p>
                <p className="text-xl font-bold text-gray-900">{filteredTestimonials.length}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {isLoading && !isRefreshing ? (
          ...
          <div className="text-center py-8">
            ...
            <h3 className="text-base font-medium text-gray-900 mb-1">No testimonials found</h3>
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">View complete testimonial information</p>
                    </div>
                  </div>
                  <button
              ...
              <div className="p-4 overflow-y-auto max-h-[calc(85vh-100px)]">
                <div className="space-y-4">
                  <div>
                    ...
                    <h3 className="text-xl font-bold text-gray-900">{selectedTestimonial.name || 'No name available'}</h3>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">Rating</label>
                    <div className="text-sm text-gray-800">{renderStars(selectedTestimonial.rating)}</div>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">Content</label>
                    <div className="bg-gray-50 rounded-lg p-3">
                      <p className="text-sm text-gray-800 leading-relaxed whitespace-pre-wrap">{selectedTestimonial.content || 'No content available'}</p>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">User Info</label>
                    <p className="text-sm text-gray-800">
                      <span className="font-medium">Name:</span> {selectedTestimonial.user?.firstName || ''} {selectedTestimonial.user?.lastName || ''}
                    </p>
                    <p className="text-sm text-gray-800">
